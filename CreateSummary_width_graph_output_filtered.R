...
#
# Look at frequency table to decide cut off for lower end connected component removal. 
#

###
GnetsFolder <- "~/Desktop/W1/20210903_Ntermtrun_repeat4/mitoYFP_DrpIF/Original_images/channel2/cells"
...
frequency_data <- read_csv(file = "output_frequency_L.csv")

to_delete <- frequency_data %>%
  filter(percent > 0.0005) %>%   #ADD FREQUENCY CUT OFF HERE
  select("Summary_output$Length")
to_delete <- as.data.frame(to_delete)
...
for (j in Gnets) {
  ...
  #merged_graph <- graph.data.frame(as.data.frame(, directed = F))
  
  G <- graph.data.frame(as.data.frame(RawDataMerge, directed=F))
  
  ###
  ### Getting rid of noise: connected components are filtered by number of nodes=2, number of edges=1 and length > length_threshold
  ###
  ### Large Noise
  ...
  for (g in connected_comps) {
    if (ecount(g)==1 && vcount(g)==2 && sum(E(g)$Length) > length_threshold) {
      for (n in V(g)$name) {
  ...
  for (i in 1:nrow(to_delete)) {  
    for (g in connected_comps) {
      if (ecount(g)==1 && vcount(g)==2 && sum(E(g)$Length) == to_delete[i,1]) {
        for (n in V(g)$name) {
          V(G)$Noise[which(V(G)$name==n)] <- TRUE
        }
      }
...
Output <- paste(GnetsFolder,"/output_filtered.csv",sep='')
OutputSummary <- paste(GnetsFolder,"/output-summary_filtered.csv",sep='')
